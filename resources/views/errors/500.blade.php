...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ __('errors.unexpected') }} — {{ config('app.name') }}</title>
    ...
    <body class="min-h-screen bg-zinc-100 font-sans text-zinc-800 antialiased dark:bg-zinc-900 dark:text-zinc-100">
        <main class="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center gap-4 px-6 text-center">
            <p class="text-sm font-medium uppercase tracking-wide text-zinc-500">500</p>
            <h1 class="text-2xl font-semibold">{{ __('errors.unexpected') }}</h1>
            <p class="text-zinc-600 dark:text-zinc-400">{{ $message ?? __('errors.unexpected') }}</p>
