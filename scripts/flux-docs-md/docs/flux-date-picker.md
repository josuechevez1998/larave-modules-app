...
> Fuente: https://fluxui.dev/components/date-picker
...
```
<flux:date-picker value="2026-07-10" />
```
...
```
<flux:date-picker mode="range">    <x-slot name="trigger">        <div class="flex flex-col sm:flex-row gap-6 sm:gap-4">            <flux:date-picker.input variant="custom" label="Start" />            <flux:date-picker.input variant="custom" label="End" />        </div>    </x-slot></flux:date-picker>
```
...
```
<flux:date-picker unavailable="2026-07-09,2026-07-11" />
```
...
Displays a calendar interface for selecting dates](/components/calendar)
[Input

Text fields for collecting user input](/components/input)
