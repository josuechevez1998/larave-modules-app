...
> Fuente: https://fluxui.dev/components/toast
...
```
Flux::toast(    text: 'Invoice created.',    link: [        'text' => 'View invoice',        'href' => route('invoices.show', $invoice),        'navigate' => true,    ],);
```

The same link options are available when triggering a toast from JavaScript.

```
$flux.toast('Invoice created.', {    link: {        text: 'View invoice',        href: '/invoices/123',        navigate: true,    },})
```
...
A flexible content container for alerts, messages, and notifications](/components/callout)
[Modal

Display temporary content in a modal dialog](/components/modal)
