...
> Fuente: https://fluxui.dev/components/editor
...
```
import Youtube from 'https://cdn.jsdelivr.net/npm/@tiptap/[email protected]/+esm'document.addEventListener('flux:editor', (e) => {    e.detail.registerExtensions([        Youtube.configure({            controls: false,            nocookie: true,        }),    ])})
```
...
Structured form field with label](/components/field)
[Textarea

Multi-line text input areas for longer form content](/components/textarea)
