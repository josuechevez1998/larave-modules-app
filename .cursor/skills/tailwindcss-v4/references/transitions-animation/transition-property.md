...
- Consulta esta referencia cuando necesites trabajar con **transition-property**.
...
```html
<div class="transition transition-colors transition-transform"></div>
```

## Valores arbitrarios

Cuando la escala no cubra el caso, utiliza la sintaxis arbitraria documentada para `transition-property`, por ejemplo una clase completa con `[...]`.
